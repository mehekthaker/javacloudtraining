...
//		
		...
		for(int i=0; i<numberOfVehicle; i++) {
			...
			{
				int directionLeft = vehicle[i].turnLeft(vehicle[i]);
				System.out.println("The changed direction is "+directionLeft);
			}
			else 
			{
				int directionRight = vehicle[i].turnRight(vehicle[i]);
				System.out.println("The changed direction is "+directionRight);
			}
			
			System.out.println("The changed speed is "+vehicle[i].changeSpeed(vehicle[i]));
			System.out.println(vehicle[i].toString());
			
			int highestIdNumber = vehicle[i].gethighestIdentificationNumber();
			System.out.println("The highest identification number is "+highestIdNumber);
		}
